...
[지난 글](/posts/07-llm3dasset/)에서 Ashfox로 만든 모델을 마인크래프트 커뮤니티에 올렸다가 혹평을 받았다고 적었다. 혼자 고치면서는 꽤 좋아졌다고 생각했는데, 실제로 게임을 하는 사람들에게는 작은 블록으로 쌓은 건축물처럼 보였던 모양이다. 날개 끝과 몸통의 굴곡을 잘게 나누다 보니, 디테일이 많아도 전체 모양은 한눈에 읽기 어려웠다.

가장 도움이 된 조언은 큰 덩어리로 형태를 잡고 픽셀 텍스처로 디테일을 표현해 보라는 이야기였다. 그래서 몸통과 머리의 비율, 귀와 꼬리의 윤곽은 큐브로 잡고, 털의 무늬와 눈은 표면에 그리는 방향으로 하네스와 예제를 다듬었다.

그렇게 만든 여우와 고블린을 9월 3일에 다시 공개했고, 이번에는 [이전보다 많이 발전했다는 반응](https://gall.dcinside.com/mgallery/board/view/?id=steve&no=390480&page=1)을 받았다. 아래에서 작은 블록의 경계가 반복되는 이전 결과와, 넓은 면에 픽셀 무늬를 입힌 여우의 표현 차이를 볼 수 있다.

<div class="post-image-comparison">
<figure>

![개선 전: 날개와 몸체에 작은 블록의 경계가 드러나는 모델](./images/before-winged-creature.png)

<figcaption>개선 전 · 작은 블록으로 나눈 형태</figcaption>
</figure>
<figure>

![개선 후: 큰 부위별 형태와 픽셀 텍스처로 표현한 여우](./images/fox-poster.png)

<figcaption>개선 후 · 큰 형태와 픽셀 텍스처</figcaption>
</figure>
</div>

서로 다른 생물이므로 같은 모델의 전후 비교는 아니다. 여기서는 형태와 디테일을 표현하는 방식의 차이를 보려고 한다.

<details>
<summary>개선 전 결과 더 보기</summary>
...
움직임은 부품과 뼈대의 연결을 따라간다. 여우의 머리·꼬리·다리가 어느 뼈대에 붙는지 정의하고, 고블린의 무기는 오른팔 아래에 연결했다. 애니메이션에서는 그 뼈대의 회전을 시간에 따라 지정한다. 무기의 형태를 수정하면서 팔에 붙어 움직이는 관계는 유지할 수 있도록 작성한 것이다. 아래는 이 연결을 적용한 고블린의 완성 모습이다.

![몸체와 팔다리, 오른팔에 연결된 무기를 보여주는 고블린](./images/goblin-poster.png)
...
다시 공개한 글에는 좋아졌다는 반응과 함께 여우의 눈을 한 칸 정도 내리면 좋겠다는 의견, 고블린의 무기 모양과 어금니 색을 손보면 좋겠다는 의견이 달렸다.
