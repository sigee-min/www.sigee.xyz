...
image: ../08-pokemonlookup/images/03-browser-view.png
...
문제는 게임 난이도 자체보다, 내가 모르는 포켓몬이 너무 많다는 점이었다. 세대가 계속 쌓이면서 처음 보는 포켓몬이나 폼이 자주 나왔고, 타입이나 종족값을 모르니 어떤 기술을 눌러야 할지 매번 다시 찾아봐야 했다.

기존 위키도 사용하기에 충분했지만, 실전 중에 같이 쓰기에는 번거로운 부분이 있었다. 검색창 포커스를 자주 다시 잡아야 했고, 초성 검색이 안 돼 이름을 끝까지 입력해야 했고, 전투에서 먼저 보고 싶은 정보가 바로 위에 보이지 않았다.

## 구현에서 집중한 부분

구현에 들어가기 전에 기준을 세 가지로 고정했다. 앱 진입 직후 입력 가능할 것, 키보드만으로 검색과 선택이 끝날 것, 선택 직후 상성 핵심 정보가 먼저 보일 것. 핵심 평가지표는 정보량이 아니라 정보를 얻기까지의 시간과 클릭 횟수였다.
...
![모바일 검색 화면](../08-pokemonlookup/images/01-mobile-search.png)
...
![모바일 상세 화면](../08-pokemonlookup/images/02-mobile-detail.png)
...
다만 포켓몬 관련 콘텐츠는 라이선스 이슈가 엄격해서 퍼블릭 서비스로 크게 운영하기는 조심스러운 면이 있다. 그래서 지금은 개인용으로 띄워서 실제 플레이할 때 쓰는 도구로 운용하는 중이다.
